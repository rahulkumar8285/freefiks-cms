                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="card">
                                <div class="card-block">
                                    <h4 class="sub-title">Update Permission</h4>
                                    <form id="formSubmit" method="POST" action="{{ route('permissions.update', $id) }}">
                                        @csrf
                                            ...
                                            <div class="col-sm-10">
                                                <select class="form-control" name="role_id">
                                                    ...
                                                    @foreach ($roles as $role)
                                                        <option  @if($role->id == $id) selected  @endif  value="{{ $role->id }}">
                                                            {{ $role->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                        ...
                                                        @foreach ($menu->submenus as $submenu)
                                                            <div class="form-check form-check-inline p-2">
                                                                <input class="form-check-input" type="checkbox" name="permissions[]"  value="{{ $submenu->url }}"
                                                                    @if($permission->contains('submenu_url', $submenu->url)) checked @endif >
